...
from azure.ai.agents.models import Agent, ThreadMessage, ThreadRun, AgentThread, ToolOutput
from azure.ai.agents.operations import ThreadsOperations
from azure.identity import DefaultAzureCredential
from azure.ai.agents.models import ToolSet, Agent, AgentThread, ThreadMessage, ThreadRun, ListSortOrder, ToolOutput
...
class CurrencyAgent:
    ...
    async def create_agent(self) -> Agent:
        ...
        if not mcp_tools:
            raise ValueError("No valid MCP tools found. Please check your MCP server configuration.")
    ...
    async def stream(self, user_query: str, context_id: str = None) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    async def _handle_tool_calls(self, run: ThreadRun, thread_id: str):
        ...
        try:
            ...
            if not self.mcp_tool_manager:
                ...
                self.mcp_tool_manager = MCPToolManager(self.mcp_server_url)
                await self.mcp_tool_manager.initialize()
            elif not self.mcp_tool_manager._connection or not self.mcp_tool_manager._connection.is_connected:
                logger.warning("MCP Tool Manager connection not available, reinitializing")
                await self.mcp_tool_manager.initialize()
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    if function_name in available_tools:
                        logger.info(f"Executing MCP tool function: {function_name} with arguments: {arguments}")
                        ...
                        if not self.mcp_tool_manager._connection:
                            ...
                            output = await self.mcp_tool_manager._connection.execute_tool(function_name, arguments)
                    ...
                    output = {"error": f"Invalid arguments JSON: {str(e)}"}
                    logger.error(f"JSON parsing error for tool {function_name}: {str(e)}")
                except Exception as e:
                    output = {"error": f"Error executing tool {function_name}: {str(e)}"}
